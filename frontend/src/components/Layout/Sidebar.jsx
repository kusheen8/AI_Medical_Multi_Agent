...
export default function Sidebar() {
  const { T, activeAgent, setActiveAgent, agentStatus, sidebarOpen, setSidebarOpen, userData } = useContext(AppContext);
  const location = useLocation();
  const navigate = useNavigate();

  // Mapping tab IDs to paths
  const routeMap = {
    dashboard: "/",
    chat: "/chat",
    doctors: "/doctors",
  ...
  const currentTab = Object.keys(routeMap).find(key => routeMap[key] === location.pathname) || "profile";
    ...
    <aside className={`sidebar${sidebarOpen ? " open" : ""}`} style={{
      ...
      <div style={{ padding: "24px 16px 18px", borderBottom: "1px solid #E8F2EE" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#7aaa94", letterSpacing: ".5px", textTransform: "uppercase", fontFamily: "'DM Sans',sans-serif" }}>{T.appSub}</div>
          </div>
        </div>
      </div>
      ...
      <nav style={{ padding: "12px 10px", flex: 1, overflowY: "auto" }}>
        {Object.entries(T.nav).map(([id, label]) => {
          const icons = { dashboard: "⬡", chat: "◫", doctors: "✦", records: "◩" };
          const isActive = currentTab === id;
          return (
            <button key={id} className={`tbtn${isActive ? " act" : ""}`}
              onClick={() => { navigate(routeMap[id]); setSidebarOpen(false); }}
              style={{
              ...
              <span style={{ fontSize: 16, opacity: .75 }}>{icons[id]}</span>
              {label}
