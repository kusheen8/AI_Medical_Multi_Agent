"""
Groq-based cloud coordinator for reasoning trace generation.

Generates ReasoningTraces from de-identified patient context using the
Groq API. All inputs are validated to contain NO PHI before sending
to the cloud.
...
class CoordinatorTimeoutError(CoordinatorError):
    """Raised when the Groq API call times out."""
    pass
...
class GroqCoordinator:
    """Cloud coordinator that generates reasoning traces via Groq.

    All patient data is de-identified before being sent to Groq.
    The coordinator produces structured reasoning instructions that
    the local agents (MedGemma/Ollama) execute against PHI.

    Attributes:
        _client: groq AsyncGroq client instance.
        _model: Groq model name to use.
    ...
    def __init__(
        ...
        self._model = getattr(settings, "GROQ_MODEL", "llama3-8b-8192")
    ...
    async def generate_reasoning_trace(
        ...
        3. Calls Groq to generate reasoning instructions
        ...
        Raises:
            ...
            CoordinatorTimeoutError: If Groq doesn't respond in time.
            CoordinatorError: On other Groq API failures.
        """
        # Step 1: De-identify
        ...
        response_text = await self._call_groq_with_retry(system_prompt, user_prompt)
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                last_error = CoordinatorTimeoutError(
                    f"Groq API timed out after {self._timeout}s (attempt {attempt + 1})"
                )
                ...
                last_error = CoordinatorError(f"Groq API error: {exc}")
                await logger.awarning(
                    "coordinator_error",
                    attempt=attempt + 1,
                    error=str(exc),
    ...
    def _parse_response(self, response_text: str, task_type: str) -> dict[str, Any]:
        """Parse Groq response into a reasoning trace dict.
